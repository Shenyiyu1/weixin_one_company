...
**所有小红书操作只能通过本项目的 `python .claude/skills/post-to-xhs/scripts/cli.py` 完成，不得使用任何外部项目的工具：**
...
发布图文或视频内容到小红书。**最后一步"点击发布"由用户手动完成，脚本只负责填写表单。**

| 命令 | 功能 |
|------|------|
| `cli.py fill-publish` | 填写图文表单（推荐，不自动发布） |
| `cli.py fill-publish-video` | 填写视频表单（不自动发布） |
| `cli.py publish` | 图文一步发布（含自动点击，已触发蜜罐拦截，不建议使用） |
| `cli.py publish-video` | 视频一步发布（已触发蜜罐，不建议使用） |
...
# 2. 检查登录状态
python .claude/skills/post-to-xhs/scripts/cli.py check-login

# 3. 登录（如需要）
python .claude/skills/post-to-xhs/scripts/cli.py login
