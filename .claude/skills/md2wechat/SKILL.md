...
- Use `inspect` / `preview` for article metadata, readiness checks, and local preview.
- Use `convert --mode ai` to generate a WeChat-compatible HTML file from Markdown (free, no API key needed).
- **For draft publishing**: delegate to `canghe-post-to-wechat`. See [Draft Publishing](#draft-publishing) below.
...
- `convert --draft` (md2wechat API mode) requires a paid API key. Unless the user has one, always use the canghe delegation path for draft publishing.
...
**md2wechat 的 `--draft` 需要付费 API Key。草稿发布统一委托给 `canghe-post-to-wechat`，它直接调用微信 API（只需 AppID + Secret）。**

### 发布流程（Markdown 文章）

```
Step 1: md2wechat inspect article.md          ← 检查元数据 & 发布就绪
Step 2: md2wechat preview article.md          ← 本地预览排版效果
Step 3: canghe wechat-api.ts article.md       ← 发布到微信草稿箱
```

### canghe 发布命令

```bash
# Markdown 直接发布（自动渲染 HTML，默认 default 主题）
npx --cache "$HOME/.npm-cache" tsx .claude/skills/canghe-post-to-wechat/scripts/wechat-api.ts article.md --cover cover.jpg

# 指定主题（default / grace / simple）
npx --cache "$HOME/.npm-cache" tsx .claude/skills/canghe-post-to-wechat/scripts/wechat-api.ts article.md --theme grace --cover cover.jpg

# 覆盖元数据
npx --cache "$HOME/.npm-cache" tsx .claude/skills/canghe-post-to-wechat/scripts/wechat-api.ts article.md \
  --title "自定义标题" --author "作者" --summary "摘要" --cover cover.jpg

# 预览模式（不实际发布）
npx --cache "$HOME/.npm-cache" tsx .claude/skills/canghe-post-to-wechat/scripts/wechat-api.ts article.md --dry-run
```

### canghe 配置

凭证文件 `.canghe-skills/.env`（项目根目录）：
...
img.save(r'cover_compressed.jpg', 'JPEG', quality=60, optimize=True)
"
```

### canghe 脚本依赖

- 运行环境：`tsx`（通过 npx 自动安装到 `$HOME/.npm-cache`）
- 脚本路径：`.claude/skills/canghe-post-to-wechat/scripts/wechat-api.ts`
- Markdown 渲染：`scripts/md/render.ts`（自动调用，3 个主题 default/grace/simple）

## Agent Rules

- Start with discovery commands before committing to a provider, theme, or prompt.
- Route by publish target first: article draft => `inspect` → `preview` → canghe `wechat-api.ts`; image post / 小绿书 / newspic => `create_image_post`.
- **Draft publishing always goes through canghe-post-to-wechat** unless the user explicitly has an md2wechat API key.
- Prefer the confirm-first flow for article work: `inspect` -> `preview` -> canghe publish.
...
- canghe-post-to-wechat 的 `wechat-api.ts` 直接调用微信 API（使用 `.canghe-skills/.env` 中的 WECHAT_APP_ID / WECHAT_APP_SECRET），不依赖第三方 Key。
