...
**所有认证操作只能通过本项目的 `python .claude/skills/post-to-xhs/scripts/cli.py` 完成，不得使用任何外部项目的工具：**
...
```bash
python .claude/skills/post-to-xhs/scripts/cli.py wait-login
```
...
```bash
python .claude/skills/post-to-xhs/scripts/cli.py send-code --phone <用户确认的手机号>
```
...
```bash
python .claude/skills/post-to-xhs/scripts/cli.py verify-code --code <用户提供的6位验证码>
```
- 自动填写验证码、点击登录。
...
```bash
python .claude/skills/post-to-xhs/scripts/cli.py delete-cookies
```
