...
**所有运营操作只能通过本项目的 `python .claude/skills/post-to-xhs/scripts/cli.py` 完成，不得使用任何外部项目的工具：**
...
| `like-feed` | 点赞笔记 |
| `favorite-feed` | 收藏笔记 |
| `fill-publish` | 填写图文表单（表单填写后由用户手动点击发布） |
| `save-draft` | 保存草稿 |
...
6. 执行发布（参考 xhs-publish 流程，脚本填写表单后用户手动点击发布）：
```bash
python .claude/skills/post-to-xhs/scripts/cli.py fill-publish \
  ...
  --tags "标签1" "标签2"
```
7. 表单填写完成后，提醒用户：**请在浏览器中手动点击「发布」按钮完成发布。**
