...
| `scripts/render-formulas.py` | Render LaTeX formulas to PNG (tech articles) |
...
| `review_enabled` | `false` | Enable AI content review before publishing |
| `review_model` | (deprecated) | Moved to `content-review/EXTEND.md`. If set here, passed as arg to content-review. |
| `review_strictness` | (deprecated) | Moved to `content-review/EXTEND.md`. If set here, passed as arg to content-review. |
...
chrome_profile_path:
review_enabled: true
# review_model and review_strictness moved to .canghe-skills/content-review/EXTEND.md
```
...
- [ ] Step 4.5: Content review (if review_enabled)
...
- `review_enabled` (default `false`) — master switch for Step 4.5
...
**Skip if**: `review_enabled` is `false` or not set (default). Also skip for image-text posts (图文/newspic).

> **Purpose**: AI reviews the HTML article before publishing. Uses a separate model via the standalone `content-review` skill.

Invoke the review skill:

```
Skill(skill="content-review", args="platform=wechat content_type=html_article")
```

If the user's `canghe-post-to-wechat/EXTEND.md` has `review_model` or `review_strictness` set, pass them as args for backward compatibility:

```
Skill(skill="content-review", args="platform=wechat content_type=html_article model=<model> strictness=<strictness>")
```

The `content-review` skill handles:
- Loading WeChat HTML rules from its reference file
- Running AI review with a separate model (configured in `content-review/EXTEND.md`)
- Presenting a structured report with scores across 4 dimensions
- User decision: Publish / Fix & Re-review (max 3 rounds) / Override / Cancel

After `content-review` returns:
- If the user chose **Publish** or **Override**: continue to Step 5
- If the user chose **Cancel**: stop the workflow
...
• media_id: [media_id]
• Review: [Passed (8.5/10) / Skipped / Overridden by user]
...
| LaTeX formula rendering for tech articles | [references/formula-rendering.md](references/formula-rendering.md) |
...
| LaTeX formulas not rendering | See [references/formula-rendering.md](references/formula-rendering.md) — requires local LaTeX + dvipng |
| Review agent fails | Check `content-review/EXTEND.md` or retry; can skip review |
| Review too strict/loose | Adjust `review_strictness` in `.canghe-skills/content-review/EXTEND.md` |
| Review model not found | Check `review_model` in `.canghe-skills/content-review/EXTEND.md` |
| base64 图片上传失败 | `wechat-api.ts` 不处理 `data:image` URI，需先用脚本提取 base64 为文件再上传 |
| 图片路径找不到 | 图片 `src` 路径必须相对于 HTML 文件所在目录（不是 cwd）。`src="figures/xxx.jpg"` 会解析为 `<html_dir>/figures/xxx.jpg` |

## 与 gzh-design 配合

当发布论文分析类文章时，建议先用 `gzh-design` skill 做微信排版（石墨极简风），再走本技能的发布流程：

1. **gzh-design** 将 `article.md` → `article_gzh_graphite.html`（inline styles + `<span leaf="">`，直接兼容微信）
2. **render-formulas.py** 渲染公式为 PNG
3. **wechat-api.ts** 上传图片 + 发布草稿

> gzh-design 产出的 HTML 已经符合微信规范，不需要走 Step 2-3 的 markdown→HTML 转换流程。直接将 HTML 文件作为 wechat-api.ts 的输入即可。

gzh-design 安装：`git clone https://ghfast.top/https://github.com/isjiamu/gzh-design-skill .claude/skills/gzh-design/`
