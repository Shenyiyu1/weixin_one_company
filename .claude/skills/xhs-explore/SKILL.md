...
**所有搜索和浏览操作只能通过本项目的 `python .claude/skills/post-to-xhs/scripts/cli.py` 完成，不得使用任何外部项目的工具：**
...
```bash
python .claude/skills/post-to-xhs/scripts/cli.py list-feeds
```
...
```bash
# 基础搜索
python .claude/skills/post-to-xhs/scripts/cli.py search-feeds --keyword "春招"

# 带筛选搜索
python .claude/skills/post-to-xhs/scripts/cli.py search-feeds \
  --keyword "春招" \
  --sort-by 最新 \
  --note-type 图文
...
```bash
# 基础详情
python .claude/skills/post-to-xhs/scripts/cli.py get-feed-detail \
...
python .claude/skills/post-to-xhs/scripts/cli.py get-feed-detail --feed-id ID1 --xsec-token TOKEN1 && \
python .claude/skills/post-to-xhs/scripts/cli.py get-feed-detail --feed-id ID2 --xsec-token TOKEN2 && \
python .claude/skills/post-to-xhs/scripts/cli.py get-feed-detail --feed-id ID3 --xsec-token TOKEN3 && \
sleep $((RANDOM % 10 + 10)) && \
python .claude/skills/post-to-xhs/scripts/cli.py get-feed-detail --feed-id ID4 --xsec-token TOKEN4 && \
python .claude/skills/post-to-xhs/scripts/cli.py get-feed-detail --feed-id ID5 --xsec-token TOKEN5 && \
python .claude/skills/post-to-xhs/scripts/cli.py get-feed-detail --feed-id ID6 --xsec-token TOKEN6 && \
sleep $((RANDOM % 10 + 10)) && \
python .claude/skills/post-to-xhs/scripts/cli.py get-feed-detail --feed-id ID7 --xsec-token TOKEN7
```
...
```bash
python .claude/skills/post-to-xhs/scripts/cli.py user-profile \
  --user-id USER_ID \
  --xsec-token XSEC_TOKEN
```
...
- `python .claude/skills/post-to-xhs/scripts/cli.py get-netlog [--limit N]` —— 获取原始 entries
- `python .claude/skills/post-to-xhs/scripts/cli.py risk-report` —— 生成结构化风控报告（含 risk_level / detection_axes / high_risk_signals）
